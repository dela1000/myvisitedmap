  ...
  function ($scope, $http, dsServices, rndServices, keyProvider, _) {
    ...
    const createMap = (dataSelected) => {
      ...
      dataSelected.forEach((country) => {
        ...
        if (countriesVisitedByYear[year]) {
          countriesVisitedByYear[year]++;
        } else {
          countriesVisitedByYear[year] = 1;
        }
        if (country.cities) {
      ...
      $scope.countriesVisitedByYear = Object.keys(countriesVisitedByYear).map((key) => {
        return {
          year: key,
          count: countriesVisitedByYear[key]
        };
      });
